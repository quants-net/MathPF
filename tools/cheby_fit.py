...
def emit_mathpf(dest="D:/Github/MathPF/src/mathpf"):
    ...
    coefficient data in the generated mills_cheby.  Writes three files into `dest`:
      _mills_coef.h : inline constexpr double[] data only (so a future C/C++ port can #include it),
    ...
    h = ["/* Auto-generated by cheby_fit.emit_mathpf -- do not edit.",
         ...
         f"inline constexpr double M_SQRT2PI   = {float(mc.M_SQRT2PI)!r};",
         f"inline constexpr double M_SQRT2PI_2 = {float(mc.M_SQRT2PI_2)!r};  /* R(0) = sqrt(pi/2) */",
         f"inline constexpr double U_MAX       = {float(mc._U_MAX)!r};  /* sqrt(DBL_MAX); R1/R3 -> 0 beyond */",
         f"inline constexpr double X_NEG_MAX   = -37.5;  /* x < this: R/R1/R3 reflection's exp(x^2/2) overflows; saturate to +inf */",
         "", carr("XCF_R", mc.XCF_R), carr("XCF_R1", mc.XCF_R1), carr("XCF_R3", mc.XCF_R3), "",
         f"inline constexpr double N_FRAC_R1 = {float(mc.N_FRAC_R1)!r};",
         f"inline constexpr int    NSEG_R1   = {int(mc.NSEG_R1)};",
         f"inline constexpr int    TMAX_R1   = {int(mc.TMAX_R1)};",
         f"inline constexpr double N_FRAC_R3 = {float(mc.N_FRAC_R3)!r};",
         f"inline constexpr int    NSEG_R3   = {int(mc.NSEG_R3)};",
         f"inline constexpr int    TMAX_R3   = {int(mc.TMAX_R3)};", "",
